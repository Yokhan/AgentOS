...
pub fn set_activity(state: &AppState, project: &str, action: &str, detail: &str) {
    if let Ok(mut acts) = state.activities.lock() {
        acts.insert(project.to_string(), json!({
            "action": action,
            "detail": detail,
    ...
    if let Ok(acts) = state.activities.lock() {
        let map: serde_json::Map<String, Value> = acts.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
        let _ = super::claude_runner::atomic_write(&tasks_file, &serde_json::to_string(&Value::Object(map)).unwrap_or_default());
    }
}
