...
pub fn financial_dashboard(state: &AppState) -> Option<String> {
    ...
    if entries.is_empty() {
        return Some("**Financial:** No income recorded yet.".to_string());
    }
    ...
    let deadline =
        chrono::NaiveDate::from_ymd_opt(2026, 6, 5).unwrap_or(chrono::Utc::now().date_naive());
    let days_left = (deadline - chrono::Utc::now().date_naive())
        .num_days()
        .max(1);
    ...
    let mut lines = vec![
        ...
        format!(
            "Need: {:.0} RUB/day ({} days left)",
            (target - total).max(0.0) / days_left as f64,
            days_left
        ),
    ];
    ...
    for e in entries.iter().rev().take(5) {
        lines.push(format!(
            "  {} {:.0} [{}] {}",
            e.ts.chars().take(10).collect::<String>(),
            e.amount,
            e.category,
            e.description.chars().take(40).collect::<String>()
        ));
    }
