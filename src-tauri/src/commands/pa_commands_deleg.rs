...
static RE_CANCEL: LazyLock<regex::Regex> =
    LazyLock::new(|| regex::Regex::new(r"\[DELEGATE_CANCEL:([^\]]+)\]").unwrap());
static RE_STATUS: LazyLock<regex::Regex> =
    LazyLock::new(|| regex::Regex::new(r"\[DELEGATE_STATUS(?::([^\]]*))?\]").unwrap());
static RE_CLEANUP: LazyLock<regex::Regex> =
    LazyLock::new(|| regex::Regex::new(r"\[DELEGATE_CLEANUP(?::(\d+))?\]").unwrap());
static RE_PRIORITY: LazyLock<regex::Regex> =
    LazyLock::new(|| regex::Regex::new(r"\[DELEGATE_PRIORITY:([^:]+):(HIGH|MED|LOW)\]").unwrap());
static RE_TIMEOUT: LazyLock<regex::Regex> =
    LazyLock::new(|| regex::Regex::new(r"\[DELEGATE_TIMEOUT:([^:]+):(\d+)\]").unwrap());
static RE_TEMPLATE_SAVE: LazyLock<regex::Regex> = LazyLock::new(|| {
    regex::Regex::new(
        r"(?s)\[DELEGATE_TEMPLATE:save:([^\]]+)\]\s*\n?(.*?)\n?\[/DELEGATE_TEMPLATE\]",
    )
    .unwrap()
});
static RE_TEMPLATE_USE: LazyLock<regex::Regex> =
    LazyLock::new(|| regex::Regex::new(r"\[DELEGATE_TEMPLATE:use:([^:]+):([^\]]+)\]").unwrap());
static RE_LOG: LazyLock<regex::Regex> =
    LazyLock::new(|| regex::Regex::new(r"\[DELEGATE_LOG(?::([^\]]*))?\]").unwrap());
static RE_DIFF: LazyLock<regex::Regex> =
    LazyLock::new(|| regex::Regex::new(r"\[DELEGATE_DIFF(?::([^\]]*))?\]").unwrap());
...
pub fn parse_delegation_commands(response: &str, state: &AppState) -> Vec<ParsedCommand> {
    ...
    if let Some(caps) = RE_BATCH.captures(response) {
        ...
        let (valid, error) = if projects.is_empty() || task.is_empty() {
            (
                false,
                Some("DELEGATE_BATCH requires projects and task".to_string()),
            )
        } else {
            validate_projects(state, &projects)
        };
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::Batch { projects, task }),
            valid,
            error,
        });
    }
    ...
    if let Some(caps) = RE_CHAIN.captures(response) {
        ...
        let (valid, error) = if project.is_empty() || steps.is_empty() {
            (
                false,
                Some("DELEGATE_CHAIN requires project and steps".to_string()),
            )
        ...
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::Chain { project, steps }),
            valid,
            error,
        });
    }
    ...
    if let Some(caps) = RE_RETRY.captures(response) {
        ...
        let context = caps
            ...
            .unwrap_or_default();
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::Retry { id, context }),
            valid: true,
            error: None,
        });
    }
    ...
    if let Some(caps) = RE_CANCEL.captures(response) {
        ...
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::Cancel { id }),
            valid,
            error,
        });
    }
    ...
    // STATUS
    for caps in RE_STATUS.captures_iter(response) {
        let filter = caps
            .get(1)
            .map(|m| m.as_str().trim().to_string())
            .unwrap_or_default();
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::Status { filter }),
            valid: true,
            error: None,
        });
    }
    ...
    if let Some(caps) = RE_CLEANUP.captures(response) {
        let hours = caps
            ...
            .unwrap_or(24);
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::Cleanup { hours }),
            valid: true,
            error: None,
        });
    }
    ...
    if let Some(caps) = RE_PRIORITY.captures(response) {
        ...
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::Priority {
                id,
                priority: pri,
            }),
            valid: true,
    ...
    if let Some(caps) = RE_TIMEOUT.captures(response) {
        ...
        let seconds = caps
            ...
            .unwrap_or(300);
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::Timeout { id, seconds }),
            valid: true,
            error: None,
        });
    }
    ...
    if let Some(caps) = RE_TEMPLATE_SAVE.captures(response) {
        ...
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::TemplateSave {
                name,
                task,
            }),
            valid,
            error: if !valid {
                Some("Template requires name and task".to_string())
            } else {
                None
            },
        });
    ...
    if let Some(caps) = RE_TEMPLATE_USE.captures(response) {
        ...
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::TemplateUse {
                name,
                projects,
            }),
            valid,
            error: if !valid {
                Some("Template use requires name and projects".to_string())
            } else {
                None
            },
        });
    ...
    if let Some(caps) = RE_LOG.captures(response) {
        let filter = caps
            ...
            .unwrap_or_default();
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::Log { filter }),
            valid: true,
            error: None,
        });
    }
    ...
    if let Some(caps) = RE_DIFF.captures(response) {
        let filter = caps
            ...
            .unwrap_or_default();
        cmds.push(ParsedCommand {
            cmd: super::pa_commands::PaCommand::DelegExt(DelegPaCommand::Diff { filter }),
            valid: true,
            error: None,
        });
    }
    ...
    for cmd in &cmds {
        if cmd.valid {
            crate::log_info!(
                "[pa_cmd_deleg] parsed valid: {:?}",
                std::mem::discriminant(&cmd.cmd)
            );
        } else {
            crate::log_warn!(
                "[pa_cmd_deleg] invalid: {}",
