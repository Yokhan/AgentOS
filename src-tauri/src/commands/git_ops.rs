...
use super::claude_runner::{safe_truncate, silent_cmd};
...
pub fn bulk_push(state: &AppState, filter: &str) -> Option<String> {
    ...
    for (name, path) in &projects {
        ...
        match output {
            Ok(o) if o.status.success() => {
                pushed += 1;
                results.push(format!("  ✓ {} pushed", name));
            }
            Ok(o) => {
                results.push(format!(
                    "  ✗ {} failed: {}",
    ...
    Some(format!(
        "**Git Push:** {} pushed, {} skipped (up-to-date)\n{}",
        pushed,
        skipped,
        results.join("\n")
    ))
        ...
        match output {
            Ok(o) if o.status.success() => {
                ...
                if text.contains("Already up to date") {
                    ...
                    updated += 1;
                    results.push(format!("  ✓ {} updated", name));
                }
            }
            Ok(o) => {
                results.push(format!(
...
pub fn status_all(state: &AppState) -> Option<String> {
    ...
    for (name, path) in &projects {
        ...
        let ahead = if git_short(path, &["status", "-b", "--porcelain"]).contains("ahead") {
            " ↑"
        } else {
            ""
        };
        let behind = if git_short(path, &["status", "-b", "--porcelain"]).contains("behind") {
            " ↓"
        } else {
            ""
        };
...
pub fn stale_branches(state: &AppState, days: u64) -> Option<String> {
    ...
    for (name, path) in &projects {
        ...
        for line in text.lines() {
            ...
            if let Ok(dt) = chrono::DateTime::parse_from_rfc3339(parts[1].trim()).or_else(|_| {
                chrono::DateTime::parse_from_str(parts[1].trim(), "%Y-%m-%d %H:%M:%S %z")
            }) {
                if dt < threshold {
                    lines.push(format!(
                        "  {} / {} ({})",
                        name,
                        branch,
                        parts[1].trim().chars().take(10).collect::<String>()
                    ));
                }
            }
        }
    ...
    if lines.is_empty() {
        return Some(format!("No stale branches (>{} days).", days));
    }
    Some(format!(
        "**Stale Branches ({}, >{} days):**\n{}",
        lines.len(),
...
pub fn git_search(state: &AppState, mode: &str, query: &str) -> Option<String> {
    ...
    for (name, path) in &projects {
        let output = match mode {
            ...
            "code" => silent_cmd("git")
                ...
                .output(),
            "file" => silent_cmd("git")
                .args(["-C", &path.to_string_lossy(), "ls-files", query])
                .output(),
            _ => continue,
        };
        if let Ok(o) = output {
            ...
            if !text_trimmed.is_empty() {
                let lines: String = text_trimmed
                    .lines()
                    .take(5)
                    .map(|l| format!("    {}", safe_truncate(l, 100)))
                    .collect::<Vec<_>>()
                    .join("\n");
                results.push(format!("  **{}:**\n{}", name, lines));
            }
    ...
    if results.is_empty() {
        return Some(format!("No results for '{}' ({}).", query, mode));
    }
    Some(format!(
        "**Search '{}' ({}):**\n{}",
        query,
...
pub fn dependency_audit(state: &AppState, filter: &str) -> Option<String> {
    let projects = if filter.is_empty() {
        get_all_projects(state)
    } else {
        vec![(filter.to_string(), state.docs_dir.join(filter))]
    };
    let mut results = Vec::new();
    ...
    for (name, path) in &projects {
        ...
        let text = output
            ...
            .unwrap_or_else(|| "audit unavailable".to_string());
        results.push(format!(
            "  **{}** ({}): {}",
            name,
            cmd,
            text.lines().next().unwrap_or("ok")
        ));
    }

    if results.is_empty() {
        return Some("No projects with supported package managers.".to_string());
    }
    Some(format!(
        "**Dependency Audit ({}):**\n{}",
        results.len(),
...
fn get_filtered_projects(state: &AppState, filter: &str) -> Vec<(String, std::path::PathBuf)> {
    ...
    if let Some(projects_in_cat) = segments.get(filter) {
        return all
            .into_iter()
            .filter(|(name, _)| projects_in_cat.contains(name))
            .collect();
    }
