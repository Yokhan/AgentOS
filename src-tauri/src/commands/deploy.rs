...
fn ssh_exec(host: &str, command: &str) -> Result<String, String> {
    let output = silent_cmd("ssh")
        .args(["-o", "ConnectTimeout=10", "-o", "StrictHostKeyChecking=accept-new", host, command])
        .output()
...
fn scp_upload(host: &str, local: &str, remote: &str) -> Result<String, String> {
    let output = silent_cmd("scp")
        .args(["-r", "-o", "ConnectTimeout=10", local, &format!("{}:{}", host, remote)])
        .output()
    ...
    if output.status.success() {
        ...
        Err(format!("SCP failed: {}", String::from_utf8_lossy(&output.stderr).trim()))
    }
}

/// DEPLOY_STATIC: SCP files to VPS with backup
pub fn deploy_static(state: &AppState, project: &str, target_path: &str, source: &str) -> Option<String> {
    let project_dir = state.validate_project(project).ok()?;
    let src = if source.is_empty() { project_dir.to_string_lossy().to_string() } else { source.to_string() };
    let host = "yokhan-vps";
...
pub fn deploy_verify(url: &str, expected: &str) -> Option<String> {
    ...
    let _time_s = lines.first().and_then(|s| s.parse::<f64>().ok()).unwrap_or(0.0);
    let code = lines.get(1).and_then(|s| s.parse::<u16>().ok()).unwrap_or(0);
...
pub fn deploy_rollback(target_path: &str) -> Option<String> {
    ...
    if backup.trim().is_empty() { return Some("**Rollback failed:** no backup found".to_string()); }

    let cmds = format!(
        "mv {} {}.rollback 2>/dev/null; mv {} {}",
        target_path, target_path, backup.trim(), target_path
    );
    match ssh_exec(host, &cmds) {
        Ok(_) => Some(format!("**Rolled back** {} from {}", target_path, backup.trim())),
...
pub fn server_exec(host: &str, command: &str) -> Option<String> {
    ...
    match ssh_exec(host, command) {
        Ok(out) => Some(format!("**{}$ {}**\n```\n{}\n```", host, command, out.chars().take(2000).collect::<String>())),
...
pub fn server_status(host: &str) -> Option<String> {
    ...
    match ssh_exec(host, cmds) {
        Ok(out) => Some(format!("**Server {}:**\n```\n{}\n```", host, out.chars().take(2000).collect::<String>())),
        Err(e) => Some(format!("**Error:** {}", e)),
    }
}
...
fn sanitize_domain(d: &str) -> Option<&str> {
    if d.chars().all(|c| c.is_alphanumeric() || c == '.' || c == '-') && d.contains('.') { Some(d) } else { None }
}
