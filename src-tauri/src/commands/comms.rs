...
pub fn daily_report(state: &AppState) -> Option<String> {
    ...
    let today_done: Vec<&crate::state::Delegation> = delegations.values()
        .filter(|d| d.ts.starts_with(&today) && d.status == crate::commands::status::DelegationStatus::Done)
        .collect();
    let today_failed: Vec<&crate::state::Delegation> = delegations.values()
        .filter(|d| d.ts.starts_with(&today) && d.status == crate::commands::status::DelegationStatus::Failed)
        .collect();

    let mut lines = vec![format!("**Daily Report — {}**", today)];
    lines.push(format!("Delegations: {} done, {} failed", today_done.len(), today_failed.len()));

    if !today_done.is_empty() {
        lines.push("Done:".to_string());
        for d in &today_done {
            lines.push(format!("  ✓ {}: {}", d.project, d.task.chars().take(60).collect::<String>()));
        }
    }
    ...
    let active: Vec<&super::strategy_models::Strategy> = strategies.iter()
    ...
    if !active.is_empty() {
        ...
        for s in &active {
            ...
            let done = s.plans.iter().flat_map(|p| &p.steps)
                .filter(|st| st.status == crate::commands::status::StepStatus::Done).count();
...
pub fn dashboard_full(state: &AppState) -> Option<String> {
    ...
    if let Ok(d) = state.delegations.lock() {
        let pending = d.values().filter(|d| d.status == crate::commands::status::DelegationStatus::Pending).count();
        let running = d.values().filter(|d| d.status == crate::commands::status::DelegationStatus::Running).count();
        sections.push(format!("**Delegations:** {} pending, {} running, {} total", pending, running, d.len()));
    }

    // Strategies
    let strategies = super::strategy_models::load_strategies(state);
    let active_strats = strategies.iter().filter(|s| s.status != crate::commands::status::StrategyStatus::Done).count();
    sections.push(format!("**Strategies:** {} active", active_strats));

    // Plans
    let plans = super::plans::load_all_plans_internal(state);
    let active_plans = plans.iter().filter(|p| p.status == crate::commands::status::PlanStatus::Active).count();
    ...
    sections.push(format!("**Cron:** {} scheduled ({} enabled)", cron.len(), enabled));
...
pub fn activity_digest(state: &AppState, period: &str) -> Option<String> {
    ...
    let mut by_project: std::collections::HashMap<String, (u32, u32)> = std::collections::HashMap::new();
    ...
    for line in content.lines() {
        if let Ok(e) = serde_json::from_str::<Value>(line) {
            ...
            if let Ok(dt) = chrono::DateTime::parse_from_rfc3339(ts)
                ...
                if dt >= threshold {
                    let project = e.get("project").and_then(|v| v.as_str()).unwrap_or("?").to_string();
                    let status = e.get("status").and_then(|v| v.as_str()).unwrap_or("");
                    let entry = by_project.entry(project).or_insert((0, 0));
                    if status == "complete" || status == "success" { entry.0 += 1; } else { entry.1 += 1; }
                }
            }
        }
    }

    if by_project.is_empty() { return Some(format!("No activity in last {}h.", hours)); }
    let lines: Vec<String> = by_project.iter()
        .map(|(p, (ok, fail))| format!("  {} — {} ok, {} fail", p, ok, fail))
        .collect();
    Some(format!("**Activity (last {}h):**\n{}", hours, lines.join("\n")))
}
...
pub fn partner_update(state: &AppState, custom: &str) -> Option<String> {
    ...
    super::jsonl::append_jsonl_logged(&orch_file,
        &json!({"ts": state.now_iso(), "role": "system", "msg": format!("[PARTNER_UPDATE]\n{}", msg)}), "partner update");
    Some(format!("**Partner Update prepared.** Use [NOTIFY:...] to send via Telegram.\n\n{}", msg))
}
...
pub fn alert_create(state: &AppState, name: &str, body: &str) -> Option<String> {
    ...
    for line in body.lines() {
        let l = line.trim();
        if l.starts_with("Check:") { check = l[6..].trim().to_string(); }
        if l.starts_with("Condition:") { condition = l[10..].trim().to_string(); }
        if l.starts_with("Action:") { action = l[7..].trim().to_string(); }
        if l.starts_with("Schedule:") { schedule = l[9..].trim().to_string(); }
    }

    alerts.push(Alert {
        name: name.to_string(), check, condition, action, schedule,
        enabled: true, created: state.now_iso(),
    });
    let _ = std::fs::write(&alerts_path, serde_json::to_string_pretty(&alerts).unwrap_or_default());
